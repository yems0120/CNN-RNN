...
def df_all_creator(data_filepath, sampling):
    ...
    df_A = pd.DataFrame(data=A, columns=A_var).drop(columns=['cycle', 'Fc'])
...
class Input_Gen(object):
    ...
    def __init__(self, df_train, df_test, cols_normalize, sequence_length, sequence_cols, sample_dir_path,
        ...
        min_max_scaler = preprocessing.MinMaxScaler(feature_range=(-1, 1))  # X_std = (X - X.min(axis=0)) / (X.max(axis=0) - X.min(axis=0))
        norm_df = pd.DataFrame(min_max_scaler.fit_transform(df_train[cols_normalize]),  # X_scaled = X_std * (max - min) + min
